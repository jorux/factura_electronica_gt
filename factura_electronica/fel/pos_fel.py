...
class ElectronicInvoice:
    ...
    def general_data(self):
        ...
        try:
            ...
            if opt_config == 'Fecha y tiempo de peticion a INFILE':
                ...
                date_invoice_inv = frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'posting_date')
                ok_time = str(frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'posting_time'))
                ok_datetime = str(date_invoice_inv)+'T'+str(datetime.datetime.strptime(ok_time.split('.')[0], "%H:%M:%S").time())

            self.__d_general = {
                "@CodigoMoneda": frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'currency'),
    ...
    def sender(self):
        ...
        try:
            ...
            self.dat_fac = frappe.db.get_values('POS Invoice', filters={'name': self.__invoice_code},
    ...
    def receiver(self):
        ...
        try:
            ...
            if len(dat_direccion) == 0:  # Si no hay direccion registrada
                ...
                elif frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'es_dpi') == True:
                    self.__d_receptor = {
                        "@CorreoReceptor": datos_default.get('email'),
                        "@IDReceptor": str((self.dat_fac[0]['nit_face_customer']).replace('/', '').replace('-', '')).upper().strip(),  # NIT => CF
                ...
                elif frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'es_pasaporte') == True:
                    self.__d_receptor = {
                        "@CorreoReceptor": datos_default.get('email'),
                        "@IDReceptor": str((self.dat_fac[0]['nit_face_customer']).replace('/', '').replace('-', '')).upper().strip(),  # NIT => CF
                        "@NombreReceptor": str(self.dat_fac[0]["customer"]),
    ...
    def items(self):
        ...
        try:
            ...
            self.__dat_items = frappe.db.get_values('POS Invoice Item', filters={'parent': str(self.__invoice_code)},
    ...
    def adendas(self):
        ...
        """
        try:
            self.__description_adenda = frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'facelec_adenda') or ''
    ...
    def sign_invoice(self):
        ...
        try:
            ...
            #frappe.log_error(f"XML Generado", self.__xml_string) - activar para ver el xml generado y debuggear
        except:
    ...
    def request_electronic_invoice(self):
        ...
        try:
            data_fac = frappe.db.get_value('POS Invoice', {'name': self.__invoice_code}, 'company')
    ...
    def upgrade_records(self):
        ...
        if frappe.db.exists('Envio FEL', {'serie_factura_original': self.__invoice_code}):
            ...
            try:
                ...
                # 01 - tabPOS Invoice: actualizacion con datos correctos
                frappe.db.sql('''UPDATE `tabPOS Invoice`
                                 SET name=%(name)s,
                ...
                # 02 - tabPOS Invoice Item: actualizacion items de la factura
                frappe.db.sql('''UPDATE `tabPOS Invoice Item` SET parent=%(name)s
                ...
                # Actualizacion de tablas que pueden ser modificadas desde POS Invoice
                ...
                # 08 - tabPOS Invoice Timesheet
                if frappe.db.exists('POS Invoice Timesheet', {'parent': serie_fac_original}):
                    frappe.db.sql('''UPDATE `tabPOS Invoice Timesheet` SET parent=%(name)s
                                    WHERE parent=%(serieFa)s''', {'name':serieFEL, 'serieFa':serie_fac_original})
                ...
                # POS Invoice Advance - Anticipos a facturas, si existe
                # 11 - tabPOS Invoice Advance
                if frappe.db.exists('POS Invoice Advance', {'parent': serie_fac_original}):
                    frappe.db.sql('''UPDATE `tabPOS Invoice Advance` SET parent=%(name)s
                                    WHERE parent=%(serieFa)s''', {'name':serieFEL, 'serieFa':serie_fac_original})


                # POS Invoice Payment - Pagos sobre a facturas, si existe
                # 12 - tabPOS Invoice Payment
                if frappe.db.exists('POS Invoice Payment', {'parent': serie_fac_original}):
                    frappe.db.sql('''UPDATE `tabPOS Invoice Payment` SET parent=%(name)s
                                    WHERE parent=%(serieFa)s''', {'name':serieFEL, 'serieFa':serie_fac_original})
                ...
                # Parece que este no enlaza directamente con POS Invoice es el POS Invoice que enlaza con este.
                ...
                # UPDATE DECLARATIONS
                if frappe.db.exists('Invoice Declaration', {'link_name': serie_fac_original, 'link_doctype': 'POS Invoice'}):
